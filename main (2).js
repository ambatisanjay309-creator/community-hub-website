...
(function () {
  ...
  function getResponse(raw) {
    ...
    KB.forEach(entry => {
      ...
      entry.keys.forEach(kw => {
        if (q.includes(kw)) score += kw.split(' ').length * 2; // phrase bonus
  ...
  document.body.insertAdjacentHTML('beforeend', `
    <div id="chatWidget" style="position:fixed;bottom:24px;right:24px;z-index:9999;font-family:var(--font-body,system-ui,sans-serif);">
      <button id="chatToggle" aria-label="Open community assistant" title="Ask the Community Assistant"
        style="width:60px;height:60px;border-radius:50%;background:var(--teal,#1e9e8e);border:none;cursor:pointer;
      ...
      <div id="chatWindow" role="dialog" aria-label="Community Assistant"
        ...
        <div style="background:var(--navy,#1a2e4a);padding:1rem 1.25rem;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
          <div style="display:flex;align-items:center;gap:.65rem;">
            <div style="width:36px;height:36px;background:var(--teal,#1e9e8e);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;">🏛</div>
            <div>
          ...
          <button id="chatClose" aria-label="Close" style="background:none;border:none;color:rgba(255,255,255,.55);cursor:pointer;font-size:1.25rem;padding:.2rem;line-height:1;">✕</button>
        </div>
        ...
        <div id="chatChips" style="padding:.5rem 1rem .4rem;display:flex;flex-wrap:wrap;gap:.4rem;border-top:1px solid #f0f0f0;flex-shrink:0;">
        ...
        <div style="padding:.65rem 1rem;border-top:1px solid #f0f0f0;display:flex;gap:.5rem;align-items:flex-end;flex-shrink:0;">
          <textarea id="chatInput" rows="1" placeholder="Ask about local resources…" aria-label="Your message"
          ...
          <button id="chatSend" aria-label="Send"
            style="width:36px;height:36px;background:var(--teal,#1e9e8e);border:none;border-radius:50%;
  ...
  function openChat()  { win.style.display = 'flex'; win.style.flexDirection = 'column'; toggle.textContent = '✕'; input.focus(); }
  function closeChat() { win.style.display = 'none'; toggle.textContent = '💬'; }
  ...
  function bubble(text, role) {
    ...
    const bot = role === 'bot';
    d.style.cssText = `background:${bot ? '#f0f7ff' : 'var(--teal,#1e9e8e)'};color:${bot ? '#1a2e4a' : '#fff'};
      border-radius:${bot ? '12px 12px 12px 3px' : '12px 12px 3px 12px'};padding:.75rem 1rem;
      font-size:.87rem;max-width:92%;align-self:${bot ? 'flex-start' : 'flex-end'};
  ...
  function send(text) {
    ...
    chips.style.display = 'none';  // hide chips after first message
    bubble(text, 'user');
    typingDots();
    sendBtn.disabled = true;
    // Slight delay so typing dots feel natural
    setTimeout(() => {
      removeTyping();
